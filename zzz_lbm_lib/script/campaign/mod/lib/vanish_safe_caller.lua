...
-- Enable the safe caller.
-- Note that this only affects script-triggered event handlers, CM callbacks, and new event listeners, so ensure this is called early.
function safe_caller.enable()
    if safe_caller.enabled then
        out("Vanish safe caller already enabled")
        return
    end
    safe_caller.enabled = true
